...
namespace OpenRA.Mods.Common.Activities
{
	...
	static class ResponsiveMoveSupport
	{
		...
		{
			landing = default;
			if (!TryGetReplacementTarget(self, replacementActivity, out var targetPosition))
				return false;
		...
		static bool TryGetReplacementTarget(Actor self, Activity replacementActivity, out WPos targetPosition)
		{
			if (replacementActivity is IResponsiveMoveTarget responsiveTarget && responsiveTarget.TryGetResponsiveMoveTarget(self, out targetPosition))
				return true;

			foreach (var childTarget in replacementActivity.ActivitiesImplementing<IResponsiveMoveTarget>())
				if (childTarget.TryGetResponsiveMoveTarget(self, out targetPosition))
					return true;

			foreach (var target in replacementActivity.GetTargets(self))
			...
			foreach (var node in replacementActivity.TargetLineNodes(self))
